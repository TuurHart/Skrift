...
GO wave is large; **skip** D2; **already live** D3; **pending a final call** (plain-words
explanations given in chat 2026-07-10): C1, C2, C4, D5, D6, B4.
...
| A10 | **Voice dictation inside the share sheet** | ✅ | Extension records only; main app transcribes on drain with retries, appends to annotation. One take (re-record replaces) | — | — | **GO** (keep) |
| A11 | **Multi-select shares** | 🐛 | All activation rules max-count 1 → **Skrift disappears from the share sheet** with 2+ items; loader reads only the first attachment per type | Raise counts + iterate all providers (→ B1–B3). Plain words: today, the moment you select 2+ photos/files, Skrift is simply *not offered* in the share sheet | M | **GO** (soft — explained in chat) |
| A12 | **Silent failures** | 🐛 | Failed video/file copy → `cancel()` — sheet closes looking like success; `CaptureInbox.write` failure ignored (completes anyway); failed file load → **empty text sheet** → saving creates a husk capture. Plain words: when an import fails, the app never tells you — the share sheet closes as if it worked and the item is just *gone* | Error state in the sheet + "Saved to Skrift ✓" confirmation (→ E3) | S–M | **GO** (soft — explained in chat) |
