...
| Audio-route-change handling (AirPods pull-out) | ✅ | n/a | `LiveRecordingService.swift` | ✅ 2026-06-12: on every route transition (pull-out / re-insert / category change) the tap is torn down and REINSTALLED in the new route's input format, buffers converted to the file's fixed write format via a persistent `AVAudioConverter`; engine restarted + amber notice names the new input. (The 06-09 restart-only fix died on device: the stale-format tap made every write fail silently.) Also exposes `LiveRecordingService.isRecordingActive` (MainActor static, true incl. paused) so the audiobook player ignores remote-play mid-recording. Unit-tested: format-change decision, converter selection, conversion continuity, active-flag lifecycle (`LiveRecordingRouteChangeTests.swift`) |
