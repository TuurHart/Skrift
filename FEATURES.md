...
| Build-your-quote selection | ✅ | n/a | `MergedCaptureView`; pure `TextCaptureSelection` + `TextCaptureMath` in `Models/TextCaptureSelection.swift` | Scroll the ~90 s window; tap a grey line's **+** to add / an end line's **✕** to drop; last line pre-picked. Now lives INSIDE the merged note-style screen (standalone `TextCaptureView` retired 2026-06-13). **No back-limit (2026-06-14): an "Earlier ↑" control extends the range further back — +8 sentences/tap from the sidecar (transcribed, unlimited to file start) / re-transcribes ~60 s more per tap when un-chunked (≈4, remaps `sel`)** |
