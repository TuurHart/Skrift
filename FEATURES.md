...
| **"People in this note" chip bar** (review) | ➖ | ✅ | `Features/Review/PeopleChipBar.swift`, `NoteDisplayView.column`, `ProcessingCoordinator.toggleAbout`/`resanitiseForNames`, `Sanitiser.detectedPeople`/`matchedSpeakers` | ✅ 2026-06-15 (chunk 3): detected people render as chips above the body; tap → toggle `aboutPeople` → re-link the body LIVE (deterministic, no LLM) + recompile + `people:` updates. ON = full name + accent; OFF = `＋ short`; a conversation's matched SPEAKERS show locked-on. Snapshot-verified all 3 states (`-snapshot-people`); detection + toggle unit-tested |
