...
- `ambiguous_names` (on each note) holds unresolved ambiguities → **needs a review-time resolver UI (Phase 5 chunk 5, NOT yet built).** **Open design fork found while starting it:** the auto-run name-links the **copy-edit** body and writes the linked text to `sanitised` (`batch_manager.py:461-472`), recording `ambiguous_names` offsets into the copy-edit text. But the **frontend body uses `enhanced_copyedit` first** (`getBestText`) while **compile/export uses `sanitised` first** — so the body the user edits is *unlinked* while the export is *linked*, and edits to the copy-edit make `sanitised` stale. Before building the resolver, decide the single source of truth for the body and where resolved `[[links]]` go (likely: make the body prefer `sanitised` once name-linking has run, or fold links into `enhanced_copyedit`). The existing `POST /api/process/sanitise/{id}/resolve` + `resolve_name_disambiguation` operate on the **transcript** (stale for this pipeline) — rework or replace. Orphaned `DisambiguationModal` + `api.ts` `resolveSanitise`/`groupOccurrences` are still around to repurpose or delete.
...
> **STATUS (resume here):** Layout decision MADE — user picked **Option B (2-pane + toolbar, no third column)** from rendered mockups, with a revised toolbar (real audio transport, no status chips, no Preview). **Chunks 1–4 built, verified, committed** (see §1). **Remaining: chunk 5 (ambiguous-name resolver — has an open design fork, see the `ambiguous_names` bullet in §1 "Key behavior changes"), chunk 6 (virtualized queue + status chips), chunk 7 (resizable panes via `react-resizable-panels` + native macOS titlebar `vibrancy`+`hiddenInset` in `electron/main.cjs`).** Chunks 6 & 7 are independent of the chunk-5 fork. The two-title chooser's dedicated **phone-title capture on upload** (§4) is still deferred — currently the "From recording" candidate is the cleaned filename.
