...
- **Phase 3 ✓ COMPLETE** (commits `5da7a2e`, `ffca320`, `fbe2699`, `2872964`): replaced the 4 racing polling loops with **one source of truth** — `useFiles` (`['files']` query of full objects, polls 1s only while something is processing) + `useCurrentBatch`. App derives the selected file + enhance-lock from it; Sidebar/Inspector consume it; edits go through optimistic cache patches (`useFilesCache`) + invalidation. Fixed the contenteditable race with a **dirty-guard** in `NoteBody` (an in-flight refetch can't revert unsaved edits). SystemStatus keeps its own 15s `/health` poll (not a file-state race). **Verified live in-browser** (Vite + preview tools): list/status load, zero console errors, selection renders body+inspector, a **blurred edit survived 3.5s of active refetches + the save with no revert**, and the enhance-lock banner was accurate.
  - ~~Heads-up: a pre-existing note was stuck at `enhance: processing`...~~ **RESOLVED** (commit `1f0312a`): a startup heal in `status_tracker.load_existing_files` now demotes any stale `processing` step on load (enhance → done when title+copy-edit+summary present, else pending), so phantom spinners/locks can't survive a restart.
- **Phase 4 ✓ COMPLETE** (commits `769f571`, `a41c6d5`, `fc936ce`, `adf5c48`): shadcn component foundation. Deduped `formatDuration` (3 copies → `lib/format.ts`; `formatDate` left alone — two intentional formats). Added `src/components/ui/` primitives (button, dialog, input, tabs, tooltip, slider, sonner, command) **adapted to the app's tokens** (bg-accent / bg-surface / text-text-*), plus deps (radix slot/slider/tabs, sonner, cmdk, react-resizable-panels, @tanstack/react-virtual). Migrated 4 bespoke modals → Dialog (Sidebar delete, Inspector RAM, AddNameModal, EnhancementTab chat-template) — gains escape/focus-trap. **SetupWizard + Settings left as-is** (full-screen flows, not dialogs); **DisambiguationModal skipped** (dead since Phase 2). Inspector `Btn` now wraps Button (call sites unchanged); `<Toaster/>` mounted. **Verified in-browser**: delete dialog opens + closes on Escape, buttons render, toaster mounted, full build passes. The not-yet-adopted primitives (input/tabs/tooltip/slider/command) + the resizable-panels/virtual deps are foundation for Phase 5.
- Verified backend-healthy + frontend-type-check-clean after every chunk. Backend currently **running** (test note "Hotel Du Vin" is in the pipeline at Ready — deletable from the app). Vite dev server may also be running (Phase 3/4 browser verification).
...
## 3. Phases 2–4 ✓ DONE — NEXT TASK is Phase 5 (Obsidian-mirror review redesign)

Phases 2 (pipeline reorder + auto-run orchestrator), 3 (one TanStack Query source of truth + contenteditable race fix) and 4 (shadcn component foundation) are complete and verified — see §1 for summaries and key behavior changes. **The next task is Phase 5** (detail in §5): the Obsidian-mirror review surface — an editable **properties block** in the MIDDLE pane (two-title chooser, significance **slider**, tag **chips** + vault autocomplete, source/date) above the editable body; **karaoke-on-body** corrector (align body words → transcript `word_timings`, fix the formatting-jump bug); a virtualized left-pane queue; and **mock BOTH right-pane layouts** (slim rail vs 2-pane + toolbar) for the user to pick from visuals. The `ui/` primitives (slider, command, tabs, input) + `react-resizable-panels` / `@tanstack/react-virtual` are now in place to build on. Phase 5 also re-wires ambiguous-name resolution (the now-unused `DisambiguationModal`) into the review step, using the `ambiguous_names` data Phase 2 records. Then Phase 6 (polish/ship). The Phase 2 detail below is a historical record.
