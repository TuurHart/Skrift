...
## 3. Phases 2 & 3 ✓ DONE — NEXT TASK is Phase 4 (shadcn component foundation)

Phase 2 (pipeline reorder + auto-run orchestrator) and Phase 3 (frontend state rebuild — one TanStack Query source of truth + contenteditable race fix) are complete and verified — see §1 for summaries and key behavior changes. **The next task is Phase 4** (detail in §5): install shadcn primitives (Button, Dialog, Input, Tabs, Tooltip, Slider, Toast/sonner, Command/cmdk — radix/cva/clsx/tailwind-merge/lucide already present; `@tanstack/react-query` now too), add `react-resizable-panels` + `@tanstack/react-virtual`, and dedupe the hand-rolled bits (`formatDuration` ×4 → `lib/format.ts`, bespoke modals → Dialog, inline `Btn`/`Section` in Inspector). Then Phase 5 (Obsidian-mirror review redesign) and Phase 6 (polish/ship). The Phase 2 detail below is a historical record.
