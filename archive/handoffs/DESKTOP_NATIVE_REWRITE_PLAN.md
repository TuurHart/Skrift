...
- [x] Phase 3 — transcription: pure post-processing (BPEMerge + ImageMarkers, host-tested) + Engines/TranscriptionService (FluidAudio Parakeet v3, app-only). Real-ASR verified end-to-end on an M4/ANE via a throwaway tool: "The quick brown fox…" (conf 0.998). FluidAudio auto-downloads the model from HF.
- [x] Phase 4 — enhancement: Engines/EnhancementService (mlx-swift-lm MLXLLM + ChatSession; copy-edit/title/summary on the raw transcript, photo-aware copy-edit via the host-tested ImageMarkerReinsert). Prompts verbatim from settings.py. App compile-verified with MLX linked (metallib built into the .app); the LLM run itself was proven in the Phase 0 spike. Fast `UnitTests` scheme keeps the host-less loop MLX-free.
- [x] Phase 5 — tags + name-linking + compile: Sanitiser (non-blocking name-linking, port of sanitisation.py), TagMatcher (deterministic tags via NLTagger, port of match_tags_in_text/extract_spoken_hashtags), Compiler (Obsidian markdown + YAML frontmatter, port of compile_file). All pure; host-tested. 49 unit tests.
- [x] Phase 6 — BatchManager auto-run: BatchRunner orchestrates transcribe→copy-edit/title/summary→tags→name-link→compile on one file; engines behind Transcribing/Enhancing protocols so it host-tests with stubs (full run + trusted-skip). 51 unit tests; app compile-verified. (Owed: wire to SwiftData + sidecar writes + a real end-to-end run in the app.)
- [ ] Phase 7 — review UI
