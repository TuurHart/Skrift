...
- **BookShare** · Share a book with someone · **planned** · ms:Enrichment · deps:[EPubAlign] · eff:M — ONE file, ONE button: a `.skriftbook` (ZIP; ZIPFoundation is already dep #2) carrying the audio, the ePub if there is one, and the whole‑book transcript + alignment as invisible plumbing. **Never** bookmarks, position or notes. Goes to anyone running Skrift over AirDrop / Files / Messages — one importer for all three. **Cut twice by Tuur 2026‑07‑30:** own devices are out ("all that is done over cloud sync" — `AudiobookCloudSync` already carries audio, cover, position, bookmarks, transcript *and* alignment between them), then all options are out ("just 1 option… no fluff"). The second cut also deleted the duration‑match test, the mismatch screen and every merge path — with audio unconditional, a bundle's transcript always arrives with the audio it was measured against. Design gotcha: Dev + Prod both *exporting* one UTI makes document routing undefined, so the type must be per‑config like the bundle ids. Mocked 2026‑07‑30 (`mocks/book-sharing.html`), **sign‑off owed, not built**. Board = `backlog.md` 📦.
