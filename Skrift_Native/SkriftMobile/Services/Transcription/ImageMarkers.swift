...
/// Shared `[[img_NNN]]` marker insertion, ported verbatim from the RN
/// `ParakeetModule` / desktop `_insert_image_markers`. Used by both the real
/// FluidAudio engine and the seeded test transcriber so the photo→offset→marker
/// pipeline is deterministically testable. Works in UTF-16 indices (NSString)
/// throughout — spot-check multibyte/emoji parity vs the Python side if it drifts.
enum ImageMarkers {
    struct TimedWord {
        let text: String
