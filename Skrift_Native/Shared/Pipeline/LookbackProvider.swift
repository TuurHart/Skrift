...
enum LookbackProvider {
    ...
    /// "Important lately" (design 2026-07-07): every Important-tier note of the
    /// last ~30 days — TIER-anchored where Looking back is TIME-anchored (a
    /// top-tier note between window anchors never surfaces there). Doubles as
    /// the wall's digital twin. Newest first, capped.
    static func importantLately(for memos: [Memo], now: Date = Date(),
        ...
        return memos
            .filter { ThreeBallScale.step(for: $0.significance) == ThreeBallScale.stepCount && journalDate($0) >= cutoff }
