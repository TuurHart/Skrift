...
enum VocabularyTermParsing {
    ...
    static func parse(_ entry: String) -> Parsed {
        ...
        // A degenerate "  : foo" with an empty canonical is treated as a bare
        // entry of the raw text (no aliases) so we never feed an empty term.
...
/// `1 - editDistance / maxLength` (the same measure its rescorer gates on). Pure
/// + unit-tested so the booster's trust guard can be reasoned about offline.
...
enum VocabularyTuning {
    static func cbw(default def: Float) -> Float {
        #if DEBUG
        if let v = ProcessInfo.processInfo.environment["SKRIFT_VOCAB_CBW"].flatMap(Float.init) { return v }
        #endif
        return def
    }

    static func minSimilarity(default def: Float) -> Float {
        #if DEBUG
        if let v = ProcessInfo.processInfo.environment["SKRIFT_VOCAB_MINSIM"].flatMap(Float.init) { return v }
        #endif
        return def
    }
}
