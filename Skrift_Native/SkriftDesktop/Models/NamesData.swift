...
// Contract-critical names types. Duplicated VERBATIM from the iOS rewrite
// (`SkriftMobile/Models/NamesData.swift`) so the on-disk schema + JSON encoding
// round-trip byte-for-byte through the phone↔Mac sync. The two native apps share
// this contract but NOT a Swift package yet (plan §8 — converge later).

/// A speaker voice profile for diarization. Multi-embedding, NEVER averaged —
/// matching is max-cosine over the list. `vector` is `[Double]` to survive JSON
/// round-trips exactly. Synced verbatim with the phone (opaque pass-through here).
...
/// Encoding is contract-faithful: `short` is always written (null when empty),
/// `voiceEmbeddings` is omitted when empty, and `deleted` is written only for
/// tombstones.
...
/// so unit tests verify the load-bearing LWW + voiceEmbeddings-union behavior
/// without a backend.
enum NamesMerge {
    ...
    /// ties default to remote. `voiceEmbeddings` are ADDITIVE — unioned across both
