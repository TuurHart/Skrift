...
| Task A: auto-sync names after voice enrollment (real bug) | 7670 | DONE-SINCE | `⭐ CloudKit-only sync epic` (line 6215) fixes this class of bug: "phone pushes NamesCloudSync/VocabularyCloudSync on edit" (commits `(hash removed by the second pass: no such commit)`/`79975a7`) — and the same file later (line 8338) confirms: "CONFIRMED BUGS fixed: names AUTO-SYNC after voice enroll (debounced push, no-op unpaired)" |
